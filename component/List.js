...
const List = ({ searchPhrase, setClickedOnBar, data }) => {
    ...
    const Item = ({ name, country, data }) => (
        <TouchableOpacity style={styles.item} onPress={() => navigation.navigate("WeatherScreen", { data: data })}>
    ...
    const renderItem = ({ item }) => {
        ...
        if (item.country.toUpperCase().includes(searchPhrase.toUpperCase().trim().replace(/\s/g, ""))) {
            return <Item name={item.name} country={item.country} />;
        }
    };
